...
int main(int argc, char *argv[])
{
  ...
  char          *end;
  if (argc > 2 && strncasecmp(argv[1], "-t", 2) == 0) {
    errno = 0;
    threads = strtoul(argv[2], &end, 10);
    if (!errno) omp_set_num_threads(threads);
  }
  ...
  for (it = 1; it <= 1; it++) {
    ...
    #pragma omp parallel for default(shared) private(j) \
                             reduction(+:norm_temp1,norm_temp2) schedule(runtime)
    for (j = 0; j < lastcol - firstcol + 1; j++) {
                      ...
                      double *rnorm)
{
  ...
  //---------------------------------------------------------------------
  #pragma omp for reduction(+:sum) nowait schedule(runtime)
  for (j = 0; j < lastcol-firstcol+1; j++) {
    suml = x[j] - r[j];
    sum  = sum + suml*suml;
  }
