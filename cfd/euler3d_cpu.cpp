...
template <typename T>
void copy_distributed(T* dst, T* src, int nelr, int nvar)
{
  ...
  for(int blk = 0; blk < nelr/block_length; ++blk)
  {
    ...
    {
      dst[i + VAR_DENSITY*nelr] = src[i + VAR_DENSITY*nelr];
      dst[i + (VAR_MOMENTUM+0)*nelr] = src[i + (VAR_MOMENTUM+0)*nelr];
      dst[i + (VAR_MOMENTUM+1)*nelr] = src[i + (VAR_MOMENTUM+1)*nelr];
      dst[i + (VAR_MOMENTUM+2)*nelr] = src[i + (VAR_MOMENTUM+2)*nelr];
      dst[i + VAR_DENSITY_ENERGY*nelr] = src[i + VAR_DENSITY_ENERGY*nelr];
    }
  }
}
#ifdef OMP_OFFLOAD
...
void dump(float* variables, int nel, int nelr)
{
...

	{
		...
		fprintf(file, "%d %d\n", nel, nelr);
		for(int i = 0; i < nel; i++) fprintf(file, "%f\n", variables[i + VAR_DENSITY*nelr]);
		fclose(file);
	}
...

	{
		...
		for(int i = 0; i < nel; i++)
		{
			for(int j = 0; j != NDIM; j++) fprintf(file, "%f ", variables[i + (VAR_MOMENTUM+j)*nelr]);
			fprintf(file, "\n");
		}
		fclose(file);
	}
...
void compute_flux(int nelr, int* elements_surrounding_elements, float* normals, float* variables, float* fluxes, float* ff_variable, float3* ff_flux_contribution_momentum_x_p, float3* ff_flux_contribution_momentum_y_p, float3* ff_flux_contribution_momentum_z_p, float3* ff_flux_contribution_density_energy_p)
{
	const float smoothing_coefficient = float(0.2f);
        float3 ff_flux_contribution_momentum_x = *ff_flux_contribution_momentum_x_p,
               ff_flux_contribution_momentum_y = *ff_flux_contribution_momentum_y_p,
               ff_flux_contribution_momentum_z = *ff_flux_contribution_momentum_z_p,
               ff_flux_contribution_density_energy = *ff_flux_contribution_density_energy_p;
...
int main(int argc, char** argv)
{
	...
	float* normals;
	{
		...
		//file >> nel;
		fscanf(file, "%d", &nel);
		nelr = block_length*((nel / block_length )+ (nel % block_length < 1 ? nel % block_length : 1));//std::min(1, nel % block_length));
		...
		{
			//file >> areas[i];
			fscanf(file, "%f", &areas[i]);
			for(int j = 0; j < NNB; j++)
			{
				//file >> elements_surrounding_elements[i + j*nelr];
				fscanf(file, "%d", &elements_surrounding_elements[i + j*nelr]);
				...
				for(int k = 0; k < NDIM; k++)
				{
					...
					fscanf(file, "%f", &normals[i + (j + k*NNB)*nelr]);
	...
	{
#ifdef _OPTIMIZED
                copy_distributed<float>(old_variables, variables, nelr, NVAR);
#else
