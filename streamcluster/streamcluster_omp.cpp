...
float pspeedy(Points *points, float z, long *kcenter, int pid, pthread_barrier_t* barrier)
{
  ...
  if( pid != 0 ) { // we are not the master threads. we wait until a center is opened.
    ...
    for(i = 1; i < points->num; i++ )  {
      float ratio = (float)lrand48() / (float)INT_MAX;
      float cost = points->p[i].cost / z;
      bool to_open = ratio < cost;
      if( to_open )  {
	(*kcenter)++;
#ifdef ENABLE_THREADS
	pthread_mutex_lock(&mutex);
...
//	#pragma omp parallel for 
	#pragma omp parallel for reduction(+: cost_of_opening_x) schedule(runtime)
  for ( i = k1; i < k2; i++ ) {
...
#endif
    pthread_t* threads = (pthread_t *)malloc(sizeof(pthread_t) * nproc);//new pthread_t[nproc];
    pkmedian_arg_t* arg = (pkmedian_arg_t *)malloc(sizeof(pkmedian_arg_t) * nproc);//new pkmedian_arg_t[nproc];
		    ...
		    long chunksize, long centersize, char* outfile )
{
  ...
  while(1) {
    ...
    if( stream->stream_error() || (numRead < (unsigned int)chunksize && !stream->stream_eof()) ) {
...
{
  char *outfilename = (char *)malloc(sizeof(char) * MAXNAMESIZE);//new char[MAXNAMESIZE];
  char *infilename = (char *)malloc(sizeof(char) * MAXNAMESIZE);//new char[MAXNAMESIZE];
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
  ...
  srand48(SEED);
  assert(n > 0 && "Invalid configuration");
  PStream stream(n);
  //PStream* stream;
  //SimStream sstream(n);
  //FileStream fstream(infilename);
  //if( n > 0 ) {
  //  stream = &sstream;
